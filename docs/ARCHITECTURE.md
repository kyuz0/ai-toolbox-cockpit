...
- `views/` owns the four top-level workflows: toolboxes, servers, models, and llama.cpp benchmarks.
...
Schema version 2 has three top-level sections:
...
- explicit `interactive`, `models`, `server`, and `benchmark` feature states.
...
- `llama_cpp`: GGUF repository records with optional profiles, MTP, vision-projector, compatibility, and benchmark metadata;
...
5. Add its model section, toolbox records, platform assignments, defaults, and all four feature states.
...
- Interactive shells, model downloads, model managers, benchmarks, and servers suspend Textual while they own the terminal.
